...
- `128 PG` для RDB пулов в `70%` от общего объёма (`186` округлены до `256`);
...
- [group_vars/all.yml](group_vars/all.yml) - общие переменные для всех узлов, а также переменная **ceph_host_osd_spec**, которая позволяет использовать все свободные диски на узлах **mon** и **mgs**;
